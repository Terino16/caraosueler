...
const TEMPLATE = readFileSync(
  join(process.cwd(), "public", "exercise-carousel-prompt-generator-template.md"),
  "utf-8",
);
 ...
 *
 * gpt-5.4-nano is instructed to skip Pro tip slides, so its output has fewer
 * sections than the total slide count.  We advance through sections only for
 * non-skipped indices.
 */
function parsePromptsFromMarkdown(
  ...
  for (let i = 0; i < slides.length; i++) {
    if (isSkipped(slides[i], i)) continue; // forced null; don't consume a section
